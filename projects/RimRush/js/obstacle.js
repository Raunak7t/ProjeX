...
function createObstacle() {
  ...
  const obsLeft = {
    ...
    set left(value) {
      obstacle.style.setProperty("--obs-left", value);
    },
    remove() {
      obstacle.remove();
    },
    obsRect() {
      return [obsUp.getBoundingClientRect(), obsDown.getBoundingClientRect()];
    },
  };
...
function createObsSeg(obsPosClass) {
  ...
  obsSeg.classList.add("obs-seg", obsPosClass);
  return obsSeg;
}
